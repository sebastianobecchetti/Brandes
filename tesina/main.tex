...
l'esigenza di quantificare la rilevanza di un singolo nodo all'interno della rete,
ovvero quanto esso sia importante nel collegare fra loro gli altri nodi lungo i
cammini minimi. Tra le misure proposte a tale scopo, una
...
Dal punto di vista del costo, la visita inserisce ed estrae ogni vertice dalla coda
$Q$ una sola volta, con contributo $O(n)$; poiché il grafo è descritto tramite
\emph{liste di adiacenza}, la scansione dei vicini esamina ogni arco una sola volta
per ciascun estremo, per un totale di $\sum_{v\in V}\deg(v)=2m=O(m)$. La BFS costa
pertanto $O(n+m)$ per sorgente. Con una matrice di adiacenza la sola scansione
salirebbe a $O(n^2)$, poiché per ogni vertice si ispezionerebbero tutte le $n$
possibili adiacenze anziché i soli vicini effettivi.
...
Quanto al costo, il ciclo estrae dalla pila ciascun vertice una sola volta e, per
ogni $w$ estratto, ne scandisce i vicini: ogni arco è così esaminato una sola volta
per ciascun estremo, per un totale di
\[
	\sum_{w\in V}\deg(w) = 2m = O(m)
\]
scansioni. Sommando l'estrazione degli $n$ vertici, la fase di accumulo costa
$O(n+m)$ per sorgente, quanto il calcolo dei cammini minimi che la precede.
L'elaborazione di una singola sorgente è dunque lineare, $O(n+m)$, e il costo
complessivo dell'algoritmo, ripetuto sulle $n$ sorgenti, è $O(nm)$ per grafi con
$m\gtrsim n$ (il sovraccarico lineare per sorgente è ripreso nel Capitolo~4).
...
estrazione e scartare le copie successive --- discende appunto
